...
InvestControl.prototype.handleIC01 = function (headNode, bodyNode, cb) {
    async.waterfall([
        ...
        function (id, cb) {
            ...
            var cond = {};
            cond.loan_id = bodyNode.loanId;
            cond.invest_money = bodyNode.money;
            cond.money = bodyNode.money;
            cond.user_id = bodyNode.userId;
            cond.time = new Date();
            cond.status = investStatus.cancel;
            cond.id = id;
            investTab.save(cond, [], function (err) {
                if (err) {
                    cb(ec.E9050);
                } else {
                    var backNode={};
