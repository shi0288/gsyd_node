...
AccountControl.prototype.handleAC01 = function (headNode, bodyNode, cb) {
    userBillService.getLastestBill(bodyNode.userId,function(err,data){
        if(err){
            cb(ec.E9999);
        }else{
            var backNode={};
            backNode.balance=data.balance;
            backNode.frozen_money=data.frozen_money;
            cb(null,backNode);
        }
    });
};
...
AccountControl.prototype.handleAC02 = function (headNode, bodyNode, cb) {
    userBillService.freezeMoney('w44444', 400, userBillCons.operatorInfo.INVEST_SUCCESS, "投资成功冻结金额",function(err,data){
        console.log(data);
        cb(null);
    });
