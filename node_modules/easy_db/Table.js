 ...
 * @param cb
 */
Table.prototype.save = function (data, options, cb, conn) {
    log.info(data);
    var self = this;
    if (self.db.type == prop.dbType.mongodb) {
 ...
 * @param cb
 */
Table.prototype.lock = function (table, id, userCb, callBack) {
    var self = this;
    var conn = self.db.pool.getConn();
    async.waterfall([
        function (cb) {
            conn.execute('SET AUTOCOMMIT=0', {}, function (err) {
                cb(err);
            });
        },
        function (cb) {
            conn.execute('BEGIN WORK', {}, function (err) {
                cb(err);
            });
        },
        function (cb) {
            conn.execute('select * from ' + table + ' where id="' + id + '" for update', {}, function (err, data) {
                cb(err);
            });
        },
        function (cb) {
        ...
        conn.execute('COMMIT WORK', {}, function (errCommit) {
            callBack(err, data);
        });
    });
...
Table.prototype.condition = function (data, parentKey) {
    ...
    for (var key in data) {
        ...
        if (!conditionArray) {
            ...
            else if (conditionArray[1] == 'like') {
                expression += self.getKvPair(parentKey, "like", data[key]);
            }
            else if (conditionArray[1] == 'between') {
                expression += self.getKvPair(parentKey, "between", data[key][0] + "'  and  '" + data[key][1]);
            }
...
Table.prototype.getKvPair = function (colName, op, value, fromUpdate) {
    ...
    if (col == undefined && colName.indexOf('.')==-1) {
        if (colName == 'rownum') {
        ...
        col = self.colList[colName.split(".")[1]];
        var exp = colName + " " + op + " ";
 ...
 * @param cb
 */
Table.prototype.like = function (data, columns, options) {
    var self = this;
    if (self.db.type == prop.dbType.mongodb) {
        return self.col.find(data, columns);
    }
...
Table.prototype.remove = function (condition, options, cb) {
    var self = this;
    if (self.db.type == prop.dbType.mongodb) {
        self.col.remove(condition, options, cb);
    }
    else {
 ...
 * @param cb
 */
Table.prototype.isExist = function (cb) {
    var self = this;
    if (self.db.type == prop.dbType.mysql) {
        var sql = "show tables like " + "'" + self.name + "'";
        var conn = self.db.pool.getConn();
        conn.execute(sql, [], cb);
    }
};
